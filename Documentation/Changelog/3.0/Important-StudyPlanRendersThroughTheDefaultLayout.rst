...
link pointing at it landed on the page and not on the element.
...
The frame wrapper, the spacing classes and the anchor are rendered by the
:file:`Layouts/Default` layout of whichever package provides
...
:file:`AcademicStudyPlan.html` keeps rendering that copy and is unaffected -
including a copy that was made to add the layout in the first place, which can
be dropped once nothing else in it was changed.
