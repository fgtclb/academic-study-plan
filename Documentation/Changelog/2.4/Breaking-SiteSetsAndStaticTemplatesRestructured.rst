...
shipped them through were four one-line :typoscript:`@import` files around two
real ones:
...
*   :file:`Configuration/TypoScript/Default/setup.typoscript` and
    ...
    :file:`Configuration/TypoScript/Includes/ContentElement.typoscript`.
...
*   The three Fluid root paths of the content element are **constants** now,
    ...
    the setup.
*   The wizard entry of the content element is registered unconditionally. It
    used to sit in a :typoscript:`[typo3.branch == "12.4"]` condition, which
    never matched on a version this release supports.
