 ...
 *
 * The value is read back as the string the database returns, which is "2.50" on every
 * DBMS - MariaDB, MySQL and PostgreSQL return their decimal column as a string, and on
 * SQLite the core derives a text column for it.
 */
final class CreditPointsTest extends AbstractAcademicStudyPlanTestCase
{
    ...
    public static function creditPointsDataProvider(): \Generator
    {
        foreach (['semester', 'module'] as $record) {
            ...
            yield $record . ': a half point' => [$table, '2.5', '2.50'];
            yield $record . ': a whole number' => [$table, '30', '30.00'];
            yield $record . ': a decimal comma' => [$table, '2,5', '2.50'];
    ...
    public function creditPointsAreStoredWithTwoDecimals(string $table, string $entered, string $stored): void
    {
        ...
        $this->assertSame($stored, (string)$row['credit_points']);
    }
}
